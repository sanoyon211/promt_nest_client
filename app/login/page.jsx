...
function LoginForm() {
  ...
  return (
    <div className="min-h-[90vh] flex items-center justify-center px-4 py-12 relative overflow-hidden">
      {/* Premium Background */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[800px] h-[600px] bg-gradient-to-br from-primary/20 via-accent/10 to-transparent rounded-full blur-[120px] pointer-events-none -z-10"></div>
      ...
      <div className="bg-surface/80 backdrop-blur-xl border border-border p-10 rounded-[32px] w-full max-w-md shadow-2xl relative">
        ...
        <div className="relative z-10">
          ...
          <p className="text-text-secondary text-center mb-8 text-sm">Log in to access your dashboard and saved prompts.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 mb-8">
            ...
            <div className="relative group">
              ...
              <input required type="password" placeholder="Password" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} className="w-full bg-background/50 border border-border rounded-2xl py-4 pl-12 pr-4 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary focus:outline-none transition-all text-text-primary placeholder-text-secondary/50" />
            </div>
        ...
        <p className="text-center text-sm text-foreground/60 mt-6">
          Don't have an account? <Link href={`/register${redirectPath !== '/dashboard' ? `?redirect=${redirectPath}` : ''}`} className="text-primary font-bold hover:underline">Sign up</Link>
        </p>
      </div>
    </div>
  );
